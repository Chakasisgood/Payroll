...
class Action
{
	...
	function save_deductions()
	{
		...
		$converted_tax_percentage = $tax_percentage / 100;
		...
		$no_of_hours_minutes = convertToMinutes($no_of_hours);
		$undertime_minutes = convertToMinutes($undertime);
		$overtime_minutes = convertToMinutes($overtime);

		$no_of_hours_decimal = $no_of_hours_minutes / 60;
		$undertime_decimal = $undertime_minutes / 60;
		$overtime_decimal = $overtime_minutes / 60;

		$grand_total = ($no_of_hours_decimal * $rate_per_hour) - ($undertime_decimal * $rate_per_hour) + ($overtime_decimal * $rate_per_hour);
		$grand_total = round($grand_total, 2);

		$withholding_tax = round($grand_total * $converted_tax_percentage, 2);
		$net_amount_due = round($grand_total - $withholding_tax, 2);
    ...
    ";
		...
		if (empty($id)) {
			$check_query = $this->db->prepare("SELECT id FROM wages WHERE employee_id = ?");
			$check_query->bind_param('s', $employee_id);
			$check_query->execute();
			...
			$query = $this->db->prepare("INSERT INTO wages SET " . $data);
			$query->bind_param('ssssssssss', $employee_id, $no_of_hours_minutes, $rate_per_hour, $undertime_minutes, $overtime_minutes, $grand_total, $tax_percentage, $withholding_tax, $net_amount_due, $remarks);
			$save = $query->execute();
			...
			$query = $this->db->prepare("UPDATE wages SET " . $data . " WHERE id = ?");
			$query->bind_param('ssssssssssi', $employee_id, $no_of_hours_minutes, $rate_per_hour, $undertime_minutes, $overtime_minutes, $grand_total, $tax_percentage, $withholding_tax, $net_amount_due, $remarks, $id);
			$save = $query->execute();
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM wages where id = " . $id);
		if ($delete)
			return 1;
	}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM payroll_items where id = " . $id);
		if ($delete)
			return 1;
	}
	function calculate_payroll()
	{
		...
		$att = $this->db->query("SELECT * FROM attendance where date(datetime_log) between '" . $pay['date_from'] . "' and '" . $pay['date_from'] . "' order by UNIX_TIMESTAMP(datetime_log) asc  ");
